...
public abstract class AppWidget extends Service {
	 ...
	 */
	public static final int ONGOING_NOTIFICATION_ID = 0xdeadbeef;
	 ...
	 * @param cls
	 *            The Service extending {@link AppWidget} that will be used to
	 ...
	 * @see #show(int)
	 */
	public static void show(Context context, Class<? extends AppWidget> cls,
			int id) {
	 ...
	 *            A Context of the application package implementing this class.
	 * @param cls
	 *            The Service extending {@link AppWidget} that is managing the
	 *            window.
	 * @param id
	 ...
	 * @see #hide(int)
	 */
	public static void hide(Context context, Class<? extends AppWidget> cls,
			int id) {
		context.startService(getShowIntent(context, cls, id));
	}
	 ...
	 * @see #close(int)
	 */
	public static void close(Context context, Class<? extends AppWidget> cls,
			int id) {
	 ...
	 * @see #closeAll()
	 */
	public static void closeAll(Context context, Class<? extends AppWidget> cls) {
	 ...
	 *            the sending window.
	 * @param toCls
	 *            The Service's class extending {@link AppWidget} that is
	...
	public static void sendData(Context context,
			Class<? extends AppWidget> toCls, int toId, int requestCode,
			Bundle data, Class<? extends AppWidget> fromCls, int fromId) {
	...
	public static Intent getSendDataIntent(Context context,
		...
		return new Intent(context, toCls).putExtra("id", toId)
				.putExtra("rc", requestCode).putExtra("data", data)
				.putExtra("fromCls", fromCls).putExtra("fromId", fromId)
	...
	protected boolean showAsScreenShot = false;
	...
	protected void onReceiveTimeTick() {
	}
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null) {
			...
			if (ACTION_SHOW.equals(action)) {
				...
				int requestCode = intent.getIntExtra("rc", 0);
				@SuppressWarnings("unchecked")
				Class<? extends AppWidget> fromCls = (Class<? extends AppWidget>) intent
	 ...
	 */
	public abstract WindowController createAndAttachView(int id,
			FrameLayout frame);

	/**
	 * Return the {@link AppWidget#LayoutParams} for the corresponding id. The
	 * system will set the layout params on the view for this StandOut window.
	 * The layout params may be reused.
	 * 
	 * 
	 * @param id
	 ...
	 * @return The {@link AppWidget#LayoutParams} corresponding to the id. The
	 *         layout params will be set on the window. The layout params
	 *         returned will be reused whenever possible, minimizing the number
	 *         of times getParams() will be called.
	 */
	public abstract AppWidgetLayoutParams getParams(int id, Window window);
	...
	public int getDefaultNotificationId() {
		return getClass().hashCode() + ONGOING_NOTIFICATION_ID;
	}

	public String getDefaultBroadcastAction() {
		return getPackageName() + "STATUS_BAR_SWITCH_CLICK_ACTION";
	}
	...
	public PopupWindow getDropDown(final int id) {
		...
		if (dropDownListItems != null) {
			...
			items = new ArrayList<AppWidget.DropDownListItem>();
		}
		...
		final PopupWindow dropDown = new PopupWindow(list,
				AppWidgetLayoutParams.WRAP_CONTENT,
				AppWidgetLayoutParams.WRAP_CONTENT, true);
	 ...
	 * @see #show(int)
	 */
	public boolean onAlreadyShown(int id, Window window) {
		return true;
	}
	 ...
	 * @see #show(int)
	 */
	public boolean onRestore(int id, Window window) {
		return false;
	}
	 ...
	 *         continue.
	 * @see #updateViewLayout(int, Window, AppWidgetLayoutParams)
	 */
	public boolean onUpdate(int id, Window window, AppWidgetLayoutParams params) {
		return false;
	}
	...
	public final synchronized Window show(int id) {
		...
		if (window.visibility == Window.VISIBILITY_VISIBLE) {
			...
			Log.d(TAG, "Tried to show(" + id
					+ ") a window that is already shown.");
			onAlreadyShown(id, window);
			return null;
		}
		...
		AppWidgetLayoutParams params = window.getLayoutParams();
		...
		if (notification != null) {
			...
			if (!startedForeground) {
				...
				Log.d(TAG, "show(id)+++++" + getDefaultNotificationId());
				startForeground(getDefaultNotificationId(), notification);
				startedForeground = true;
			} else {
				// update notification if shown before
				Log.d(TAG, "show(id)-----" + getDefaultNotificationId());
				mNotificationManager.notify(getDefaultNotificationId(),
						notification);
			}
			// mNotificationManager.notify(0xdeadbeef, notification);
		} else {
	 ...
	 * @return The window shown.
	 */
	public final synchronized Window restore(int id, Uri uri) {
		Window cachedWindow = getWindow(id);
		final Window window;
		...
		if (cachedWindow != null) {
			...
			throw new IllegalStateException("Tried to restore(" + id
					+ ") a window that is lost.");
		}

		if (onRestore(id, window)) {
			Log.d(TAG, "Window " + id + " restore cancelled by implementation.");
			return null;
		}
	...
	public final synchronized void hide(int id) {
		...
		if (window == null) {
			...
			// + ") a null window.");
			Log.d(TAG, "Tried to hide(" + id + ") a null window.");
			return;
		}
		...
		if (window.visibility == Window.VISIBILITY_GONE) {
			...
			Log.d(TAG, "Tried to hide(" + id + ") a window that is not shown.");
			return;
		}
		...
		if (isSet(window.flags, StandOutFlags.FLAG_WINDOW_HIDE_ENABLE)) {
	...
	public final synchronized void close(final int id) {
		...
		if (window == null) {
			...
			// + ") a null window.");
			Log.d(TAG, "Tried to close(" + id + ") a null window.");
			return;
		}
			...
			if (animation != null) {
				animation.setAnimationListener(new AnimationListener() {
					...
					public void onAnimationEnd(Animation animation) {
						...
						sWindowCache.removeCache(id, AppWidget.this.getClass());
	...
	protected void showAsScreenShot(final int id) {
		showAsScreenShot = true;
		show(id);
	 ...
	 *            window.
	 */
	public final void sendData(int fromId, Class<? extends AppWidget> toCls,
			int toId, int requestCode, Bundle data) {
		AppWidget.sendData(this, toCls, toId, requestCode, data, getClass(),
				fromId);
	}
	...
	public final synchronized boolean focus(int id) {
		...
		if (!isSet(window.flags, StandOutFlags.FLAG_WINDOW_FOCUSABLE_DISABLE)) {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		Log.d("Window", isPortrait ? "PORTRAIT" : "LANDSCAPE");
	...
	public boolean onTouchHandleMove(int id, Window window, View view,
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (window.touchInfo.moving
				...
				if (isSet(window.flags, StandOutFlags.FLAG_BODY_MOVE_ENABLE)) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (event.getPointerCount() == 1) {
				...
				if (tap
					...
					AppWidget.this.bringToFront(id);
				}
			}

			// bring to front on touch
			else if (isSet(window.flags,
	...
	public boolean onTouchHandleResize(int id, Window window, View view,
			MotionEvent event) {
		AppWidgetLayoutParams params = (AppWidgetLayoutParams) window
				.getLayoutParams();
	 ...
	 */
	public void updateViewLayout(int id, AppWidgetLayoutParams params) {
		Window window = getWindow(id);
	...
	protected class AnimationEndListener implements AnimationListener {
	 ...
	 * 
	 */
	public class AppWidgetLayoutParams extends WindowManager.LayoutParams {
		/**
		 ...
		 *            The id of the window.
		 */
		public AppWidgetLayoutParams(int id) {
			super(200, 200, TYPE_PHONE,
					AppWidgetLayoutParams.FLAG_NOT_TOUCH_MODAL
							| AppWidgetLayoutParams.FLAG_WATCH_OUTSIDE_TOUCH,
		 ...
		 */
		public AppWidgetLayoutParams(int id, int w, int h) {
			this(id);
			width = w;
			height = h;
		}
		 ...
		 */
		public AppWidgetLayoutParams(int id, int w, int h, int xpos, int ypos) {
			this(id, w, h);
		...
		public void setFocusFlag(boolean focused) {
			if (focused) {
				flags = flags ^ AppWidgetLayoutParams.FLAG_NOT_FOCUSABLE;
			} else {
				flags = flags | AppWidgetLayoutParams.FLAG_NOT_FOCUSABLE;
			}
		}
